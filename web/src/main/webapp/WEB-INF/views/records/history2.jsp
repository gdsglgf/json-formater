...
<body>
	...
	<div class="container-fluid">
		...
		<div id="content" class="row col-md-10 col-md-offset-1" style="display:none;">
			<table class="table table-striped">
				...
				<c:forEach var="item" items="${rows}" varStatus="status">
					<tr>
						...
						<td>${item.content}</td>
						<td><button type="button" class="btn btn-success" data-toggle="modal" data-target="#formatModal" onclick="formatJSON(this)">Format</button></td>
					</tr>
				</c:forEach>
			</table>
			<p class="text-success" id="counts">Find ${length} record(s).</p>
		</div>
	...
	<script type="text/javascript">
		...
		function formatJSON(o) {
			...
			$id("format-content").innerHTML = "<PRE class='CodeContainer'>"
					+ html + "</PRE>";
		}
	</script>
